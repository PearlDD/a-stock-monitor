...
class TestGetRealtimeQuotes:
    @pytest.mark.asyncio
    async def test_returns_filtered_quotes(self, provider: AKShareTHSProvider):
    ...
    @pytest.mark.asyncio
    async def test_returns_empty_on_error(self, provider: AKShareTHSProvider):
        with patch(
            "akshare.stock_zh_a_spot_em", side_effect=Exception("Network error")
        ):
            quotes = await provider.get_realtime_quotes(["600519"])
            assert quotes == []

    @pytest.mark.asyncio
    async def test_empty_codes_returns_empty(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame(
            {
                "代码": ["600519"],
                "名称": ["贵州茅台"],
...
class TestGetStockInfo:
    @pytest.mark.asyncio
    async def test_returns_info(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame(
            {
                "item": ["股票简称", "行业", "上市时间"],
                "value": ["贵州茅台", "白酒", "2001-08-27"],
            }
        )
        with patch("akshare.stock_individual_info_em", return_value=mock_df):
            ...
            assert info.list_date == "2001-08-27"

    @pytest.mark.asyncio
    async def test_returns_none_on_error(self, provider: AKShareTHSProvider):
        with patch(
...
class TestGetStockNews:
    @pytest.mark.asyncio
    async def test_returns_news(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame(
            {
                "新闻标题": ["茅台涨停", "茅台业绩"],
                "新闻来源": ["新浪", "东财"],
    ...
    @pytest.mark.asyncio
    async def test_returns_empty_on_error(self, provider: AKShareTHSProvider):
        with patch("akshare.stock_news_em", side_effect=Exception("fail")):
            news = await provider.get_stock_news("600519")
            assert news == []

    @pytest.mark.asyncio
    async def test_limit_respected(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame(
            {
...
class TestGetFinancialSummary:
    @pytest.mark.asyncio
    async def test_returns_summary(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame(
            {
                "股票简称": ["贵州茅台"],
                "市盈率": [35.2],
    ...
    @pytest.mark.asyncio
    async def test_returns_none_on_empty(self, provider: AKShareTHSProvider):
        mock_df = pd.DataFrame()
        with patch("akshare.stock_financial_abstract_ths", return_value=mock_df):
            summary = await provider.get_financial_summary("600519")
